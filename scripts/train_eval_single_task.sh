...
for file in "${files[@]}"; do
  ...
  python -m src.social_llama.training.dpo --individual_task=$file --base_model=$model_name --model_name_or_path="./sft/${model_name}_socket_${file}/final_checkpoint"
  python -m src.social_llama.training.merge_peft_adapter --base_model=$model_name --adapter_model_name="./dpo/${model_name}_socket_${file}/final_checkpoint" --output_name="models/individual/${model_name}_${file}"
  python -m src.social_llama.evaluation.evaluator "models/individual/${model_name}_${file}" $file
done
